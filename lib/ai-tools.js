      ...
      return new Promise((resolve, reject) => {
        qbo.getInvoice(invoiceId, (err, invoice) => {
          if (err) {
            ...
            if (errorCode === '610' || errorMsg.includes('Object Not Found') || errorMsg.includes('not found')) {
              resolve({
                success: false,
                error: `Invoice with ID ${invoiceId} not found`,
                invoiceId: invoiceId,
                message: `The invoice with ID ${invoiceId} does not exist in QuickBooks. This could be because the ID is incorrect, the invoice was deleted, or it's a different transaction type (like a Bill instead of an Invoice).`
              });
            } else {
              resolve({
                success: false,
                error: `Failed to fetch invoice ${invoiceId}: ${errorMsg}`,
                invoiceId: invoiceId,
                message: `Unable to retrieve invoice ${invoiceId} due to an error: ${errorMsg}`
              });
            }
          } else if (!invoice) {
            resolve({
              ...
              error: `Invoice with ID ${invoiceId} not found`,
              invoiceId: invoiceId,
              message: `The invoice with ID ${invoiceId} does not exist in QuickBooks.`
            });
          } else {
            resolve({
              success: true,
